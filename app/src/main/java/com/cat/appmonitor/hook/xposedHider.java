...
import com.cat.appmonitor.util.FilterXpInputStream;
...
// 来自 XposedHider 项目 https://github.com/KagurazakaHanabi/XposedHider
class xposedHider extends XHook{

    private static xposedHider hider;

    public static xposedHider getInstance() {
        if (hider == null) {
            hider = new xposedHider();
        }
        return hider;
    }
                ...
                new XC_MethodHook() {
                    ...
                    protected void beforeHookedMethod(MethodHookParam param) {
                        ...
                        boolean shouldDo = path.matches("/proc/[0-9]+/maps") ||
                                (path.toLowerCase().contains(Config.KW_XPOSED) &&
        ...
        XC_MethodHook hookStack = new XC_MethodHook() {
            ...
            protected void afterHookedMethod(MethodHookParam param) {
                ...
                for (StackTraceElement element : elements) {
                    if (!element.getClassName().toLowerCase().contains(Config.KW_XPOSED)) {
                        clone.add(element);
                    }
                }
                ...
                new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        ...
                        for (int i = 0; i < len; i++) {
                            ...
                            if (!app.packageName.toLowerCase().contains(Config.KW_XPOSED)) {
                                clone.add(app);
                            }
                        }
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        ...
                        for (int i = 0; i < len; i++) {
                            ...
                            boolean shouldRemove = app.metaData != null && app.metaData.getBoolean("xposedmodule") ||
                                    app.packageName != null && app.packageName.toLowerCase().contains(Config.KW_XPOSED) ||
                                    app.className != null && app.className.toLowerCase().contains(Config.KW_XPOSED) ||
                                    app.processName != null && app.processName.toLowerCase().contains(Config.KW_XPOSED);
                            if (!shouldRemove) {
                                clone.add(app);
                            }
                        }
        ...
        XposedHelpers.findAndHookMethod(
                ...
                new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        ...
                        for (String f : fs) {
                            if (!f.toLowerCase().contains(Config.KW_XPOSED) && !f.equals("su")) {
                                list.add(f);
                            }
                        }
                ...
                new XC_MethodHook() {
                    ...
                    private String filter(String s) {
                        ...
                        for (int i = 0; i < list.size(); i++) {
                            if (!list.get(i).toLowerCase().contains(Config.KW_XPOSED)) {
